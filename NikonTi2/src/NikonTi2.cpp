...
std::vector<std::shared_ptr<DiscreteMovableComponent>> NikonTi2::getDiscreteMovableComponents() {
    ...
    if (hasMotorizedDichroic()) {
        components.push_back(std::make_shared<DiscreteMovableComponentFunctor>(
            "Dichroic Mirror", listDichroicMirrors(),
            [this](const std::string& name) { setDichroicMirror(name); }
        ));
    }
    if (hasFilterWheel()) {
        components.push_back(std::make_shared<DiscreteMovableComponentFunctor>(
            "Barrier Filter", listAvailableFilters(),
...
std::vector<std::shared_ptr<MotorizedStage>> NikonTi2::getMotorizedStages() {
    std::vector<std::shared_ptr<MotorizedStage>> stages;
    if (hasMotorizedStage()) {
        bool x = (_connectedAccessoryMask & MIC_DATA_MASK_XPOSITION) != 0;
        bool y = (_connectedAccessoryMask & MIC_DATA_MASK_YPOSITION) != 0;
        bool z = (_connectedAccessoryMask & MIC_DATA_MASK_ZPOSITION) != 0;
...
MotorizedStage::Position NikonTi2::getStagePosition() {
    lx_result err;
    ...
    return std::make_tuple(x, y, z, pfsIsActive, pfsOffset);
}

void NikonTi2::setStagePosition(const MotorizedStage::Position& pos) {
    lx_result err;
