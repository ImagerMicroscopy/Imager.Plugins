...
std::string ConvertWCSToMBS(const wchar_t* pstr) {
    return ConvertWCSToMBS(pstr, wcslen(pstr));
}

std::string BStrToStdString(bstr_t& str) {
	int wslen = ::SysStringLen(str.GetBSTR());
	return ConvertWCSToMBS((wchar_t*)str, wslen);
}
